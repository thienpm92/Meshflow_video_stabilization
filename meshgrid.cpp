...
MeshGrid::MeshGrid(int width, int height, double quadWidth, double quadHeight, double w){
...
cv::Mat MeshGrid::WarpingGlobal(const cv::Mat srcImg){
    cv::Mat warpImg;
    warpImg = cv::Mat::zeros( srcImg.size(), CV_8UC3 );
    vector<cv::Point2f> src, dst;
    for (int i = 0; i < m_meshheight; i ++)	{
        for (int j = 0; j < m_meshwidth; j ++)		{
            cv::Point2f p0 = m_mesh.getVertex(i, j);
            src.push_back(p0);
            cv::Point2f q0 = m_warpedmesh.getVertex(i, j);
            dst.push_back(q0);
        }
    }
    ...
    for (int i = 0; i < srcImg.rows; i ++) {
        for (int j = 0; j < srcImg.cols; j ++) {
            ...
            X = (int)X*W;
            Y = (int)Y*W;
            cv::Vec3b color = srcImg.at<cv::Vec3b>(cv::Point(j,i));
            if(X>=0 && X<srcImg.cols && Y>=0 && Y< srcImg.rows)
            {
                warpImg.at<cv::Vec3b>(cv::Point(X,Y)) = color;
            }
        }
    }
    return warpImg;
...
cv::Mat MeshGrid::Warping(const cv::Mat srcImg){
    cv::Mat warpImg;
    warpImg = cv::Mat::zeros( srcImg.size(), CV_8UC3 );
    for (int i = 1; i < m_meshheight; i ++)	{
        for (int j = 1; j < m_meshwidth; j ++)		{
            ...
            quadWarp(srcImg, warpImg, quad1, quad2);
        }
    }
    return warpImg;
}

void MeshGrid::quadWarp( const cv::Mat src, cv::Mat dst, const Quad &q1, const Quad &q2){
    ...
    for (int i = miny; i < maxy; i ++) {
        for (int j = minx; j < maxx; j ++) {
            ...
            {
                cv::Vec3b check = dst.at<cv::Vec3b>(cv::Point(X, Y));
                if(check.val[0]==0 && check.val[1]==0 && check.val[2]==0)
...
vector<double> MeshGrid::CreateDataCons(){
...
void MeshGrid::CreateSmoothCons(){
    rowCount = 0;
    int i=0;
    int j=0;
